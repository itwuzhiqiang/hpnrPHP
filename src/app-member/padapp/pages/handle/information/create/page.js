		...
		if (!check) {
			...
			if (apiNo) {
				this.state.handleDriverInfo.setParam('party_idnum', apiNo).fetch((json) => {
					if (json) {
						if (json.data.party_name) {
							if (json.data.party_name == apiName) {
								this.setState({
									checkName: 2,
								});
								if (apiPn) {
									this.state.handleCarInfo.setParam('vlp_no', apiPn).fetch((data) => {
										...
										if (data) {
											this.setState({
												...
												show: true,
												check: true,
												vehicle_typeStr: json.data.vehicle_typeStr ? json.data.vehicle_typeStr : json.data.vehicle_type,
												ljjf: json.data.ljjf ? json.data.ljjf : 0,
												vehicleStatus: json.data.status,
											})
										} else {
											judge1 = 0;
										}
