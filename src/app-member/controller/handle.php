...
class Controller_Handle extends Controller_Abstract {
	...
	public function doReport(PadMvcRequest $request, PadMvcResponse $response) {
		...
		$rpaAddress = urlencode($request->param('rpaAddress'));
		$points = json_decode($request->param('points'));

		$requestType = 'post';
		$url = config('domain.rpapi') . '/accident/v1/app/accident/report?accountToken=' .
			$accountToken . '&rpaType=' . $type . '&coordsys=baidu&rpaLongitude=' . $points->lng . '&rpaLatitude='
			. $points->lat . '&rpaAddress=' . $rpaAddress;
		$data = $response->helper->urlRequest($requestType, $url)['data'];
...
//var_dump($url);
	...
	public function doAccidentType(PadMvcRequest $request, PadMvcResponse $response) {
		...
		$requestType = 'get';
		$url = config('domain.rpapi') . '/accident/v1/general/accidentType?state=' . $state . '&atFatherId=' . $atFatherId;
	 ...
	 * 事故责任有争议(提交远程交警定责)
	 */
	public function doCollection(PadMvcRequest $request, PadMvcResponse $response) {
		$rpaId = $request->param('rpaId');
		$accountToken = $this->memberToken;
		$requestType = 'post';
		$url = config('domain.rpapi') . '/accident/v1/app/accident/collection?rpaId=' . $rpaId . '&accountToken=' . $accountToken;
	 ...
	 * 事故责任无争议(车主自行协商定责)
	 */
	public function doUnCollection(PadMvcRequest $request, PadMvcResponse $response) {
		$accountToken = $this->memberToken;
		$rpaId = $request->param('rpaId');
		$accident = json_decode($request->param('accident'));
		$rpaResponsibility = $request->param('rpaResponsibility');
		...
		$arr['rapConciliation'] = $rapConciliation;
		$arr['partyInputs'] = $partyInputs;
		$arr['rpaMangerInstructions'] = isset($accident->data->atRegulations) ? $accident->data->atRegulations : '其它';
		$arr['rpaManger'] = $accident->data->atId;
		$arr['rpaMangerText'] = $accident->data->atName;

		$url = config('domain.rpapi') . '/accident/v1/app/accident/unCollection?accountToken=' . $accountToken;
...
//			var_dump($result);
	 ...
	 * 事故情形详情
	 */
	public function doAccidentTypeDetails(PadMvcRequest $request, PadMvcResponse $response) {
		$atId = $request->param('atId');
		$requestType = 'get';
		$url = config('domain.rpapi') . '/accident/v1/general/accidentTypeDetails?atId=' . trim($atId, ',');
